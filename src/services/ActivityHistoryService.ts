...
/**
 * Service for managing the history of activity completions.
 * Handles:
 * - Retrieving activity history for specific dates or date ranges
 * - Generating and initializing history records for new activities
 * - Setting activity completion status
 * - Managing historical data storage and retrieval
...
export class ActivityHistoryService {
    ...
    static async getHistoryForDate(date: Date): Promise<ActivityHistory[]> {
        const key = `history_${startOfDay(date).toISOString()}`;
        try {
    ...
    static async setActivityStatus(activityId: string, date: Date, isDone: boolean): Promise<void> {
        const key = `history_${startOfDay(date).toISOString()}`;
        try {
            const existingHistory = await this.getHistoryForDate(date);
            const historyIndex = existingHistory.findIndex(h => h.activityId === activityId);

            if (historyIndex >= 0) {
                existingHistory[historyIndex].isDone = isDone;
            } else {
            ...
            await AsyncStorage.setItem(key, JSON.stringify(existingHistory));
        } catch (error) {
